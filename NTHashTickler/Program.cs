// NTHashTickler 
// Author: Ryan Ries myotherpcisacloud.com
// Multi-threaded password hash brute force tool.
...
namespace NTHashTickler
{
    class Program
    {
        ...
        static byte[] validChars = { 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 
        ...
        static IntPtr phAlgorithmProvider = IntPtr.Zero;
        static ulong hashesGenerated = 0;
        ...
        {
            NativeMethods.NTStatus ntStatus = 0;
            ...
            if (args[0].Trim().Length != 32 || !Regex.IsMatch(args[0].Trim(), @"\A\b[0-9a-fA-F]+\b\Z"))
            {
                Console.WriteLine(HelpText());
                return;
            }

            inputHashString = args[0].Trim().ToUpper();
            inputHashBytes = StringToByteArray(inputHashString);            

            if (args.Length == 2)
            {                
                if (!short.TryParse(args[1].Trim(), out maxPasswordLength))
                {
                    Console.WriteLine(HelpText());
                    return;
                }
                if (maxPasswordLength < 1 || maxPasswordLength > 127)
                {
                    Console.WriteLine(HelpText());
                    return;
                }
            ...
            ntStatus = NativeMethods.BCryptOpenAlgorithmProvider(out phAlgorithmProvider, @"MD4", null, 0);
            if (ntStatus != NativeMethods.NTStatus.STATUS_SUCCESS)
            {
                Console.WriteLine("ERROR: BCryptOpenAlgorithmProvider returned " + ntStatus);
                return;
            }

            Console.WriteLine(Environment.ProcessorCount + " CPUs found.");
            Console.WriteLine("Max password length is " + maxPasswordLength + " characters.");
            Console.WriteLine("Searching for " + inputHashString + " as MD4(UnicodeString).");
            Console.WriteLine("Hashing will now commence. Press any key to interrupt the program.");
            ...
            Parallel.For(0, Environment.ProcessorCount, cpu =>
                {
                    ...
                    while (true)
                    {
                        ...
                        NativeMethods.BCryptFinishHash(phHash, dataOut, dataOut.Length, 0);
                        NativeMethods.BCryptDestroyHash(phHash);
                        threadLocalHashesGenerated++;

                        if (NativeMethods.memcmp(inputHashBytes, dataOut, 16) == 0)
                        {
                            Console.WriteLine();
                            Console.WriteLine("CPU {0}: Match found!", cpu);
                            Console.WriteLine("{0} produces the NT hash {1}.", Encoding.Unicode.GetString(dataIn), inputHashString);
                            Interlocked.Increment(ref shouldStop);
                        }                                                     
                    }
        ...
        public static byte[] StringToByteArray(string hex)
        {
            ...
            for (int index = 0; index < output.Length; index++)
            {
                ...
                output[index] = byte.Parse(byteValue, System.Globalization.NumberStyles.HexNumber, System.Globalization.CultureInfo.InvariantCulture);
            }
            return output;
        }
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("\nNTHashTickler v" + version.ToString());
    ...
    class CryptoRNG : RandomNumberGenerator
    {
    ...
    class NativeMethods
    {
        ...
        public static extern NTStatus BCryptOpenAlgorithmProvider(
            ...
            [In, Optional, MarshalAs(UnmanagedType.LPWStr)] string pszImplementation,
            [In] UInt32 dwFlags);
